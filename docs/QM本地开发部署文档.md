...
QM 上游是**服务端渲染的多服务架构**，但在 **dev-instance 下实际只起 3 个 node 子进程 + 1 个进程内插件**——admin 不是独立进程，它由 web-ui 托管：

```
        ┌── portal : 8129（唯一公网入口 / IAP）
        │      ├── /        → web-ui : 8097（SPA + BFF）
        │      └── /admin/  → web-ui : 8097（治理台，由 web-ui 进程动态挂载）
        └── core : 8081（headless agent core）
               └── Slack 插件（Socket Mode，跑在 core 进程内）
        Postgres（Docker 容器，sessions/memory/queue）
```

> 端口按 slot 序号偏移（`scripts/dev/lib/pool.ts:69` 的 `slotPorts`）：**pool1 = core 8081 / web 8097 / portal 8129**。换用其他 slot 时这三个端口同步平移。
...
- **web-ui / admin**：两个不同形态的前端（web-ui 是真 SPA，admin 是服务端渲染小服务）。**dev 下 admin 不单独起进程**，由 web-ui 在 `/admin/*` 前缀动态挂载（`plugins/web-ui/server/index.ts:2885`）。
...
> ⚠️ **这些进程不是手动各自启动的**。dev-instance 有一个 **supervisor** 统一管理：自动装依赖、构建 web-ui、拉起 Postgres、启动 **3 个子进程（core / web / portal）**、做健康检查。子进程清单硬编码在 `scripts/dev/lib/types.ts:16` 的 `CHILD_ORDER`，实际构造在 `scripts/dev/supervisor/specs.ts:30-103`。
>
> `admin` 与 `auth` 在 dev 下都不是独立进程：admin 由 web-ui 托管，auth 完全不启动（登录走 `PORTAL_LOCAL_AUTH_BYPASS` 本地旁路）。
...
- `DEV_INSTANCE_IDLE_HOURS=0`：禁用 24 小时空闲自动回收（见第 11 节，防止闲置后实例自动消失）。
...
3. 启动 core / web / portal 三个子进程并做健康检查（admin 由 web-ui 托管，不单独起进程）。
...
[ok] dev instance up -- slot pool1 (browser only -- Slack off)
   ...
   portal : http://localhost:8129  -> prod-style front door: the assistant at / and /admin
   core   : http://localhost:8081  (org=acme, session_store=postgres, run_store=postgres)
   web    : http://localhost:8129/  (direct: http://localhost:8097)
   admin  : http://localhost:8129/admin/   (direct: http://localhost:8097/admin/)
```
...
### 11.2 ⚠️ 24 小时空闲自动回收（默认开启）

dev-instance 的 supervisor 有一个 **`IDLE_HOURS`（默认 24 小时）自动回收**机制：若 24 小时内无活动，supervisor 会 `idle self-teardown` 自己关停并删除运行状态。

默认值定义在 `scripts/dev/supervisor/main.ts:33-42`：未设置 `DEV_INSTANCE_IDLE_HOURS` 时为 `24`，非法值会告警并回落到 24。
...
| 端口 502 / status 显示 0 slots | 24 小时空闲自动回收 | 加 `DEV_INSTANCE_IDLE_HOURS=0` 重启 |
