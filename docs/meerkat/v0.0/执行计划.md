...
- L1/L2 写 session lock（独立 `DurableMap<{harnessId, modelId}>`，key = session ID），不用内存版；不写 `setRuntimeSelectionLatest`（会被 UI 手动选模型覆盖）
...
### Task 3: Orchestrator 集成 + Session Lock + Fallback 逻辑
...
- Modify: `src/api/app-sessions.ts`

**Interfaces:**
- Consumes: `SensitivityClassifier` type, `createSensitivityClassifier()` (Task 2)
- Produces: `SessionLockStore` — 独立 `DurableMap<{ sessionId: string; harnessId: string; modelId: string }>`，key = session ID
- Trigger: orchestrator 在 turn 开始时先查锁（step 1），分类器在 security screener 之后（step 3）
- Produces: 分类 verdict → 覆写 `input.harness` + `input.model` + 写入 session lock；sidecar 不可达 → fallback
...
| `input.session.id` | handleTurn 参数 `input.session.id` | 已存在 |
| `actor.id` | handleTurn 参数 `input.actor.id` | 已存在 |
| `deps.sessionLockStore` | 新增字段 | 本 Task 新增 |
...
- [x] **Step 2: 在 wiring.ts 中构造 sessionLockStore + 分类器并注入 deps**

```typescript
import { createSensitivityClassifier } from "../security/sensitivity-classifier.ts";

const sessionLockStore = artifactMap<{ harnessId: string; modelId: string }>("session_locks");
...
在 `OrchestratorDeps` 对象中追加: `sensitivityClassifier`, `classifierFallbackModel`, `classifierFallbackHarness`, `sessionLockStore`。`sessionLockStore` 同时注入到 app deps（用于 fork 拷贝锁）。
...
sessionLockStore?: DurableMap<{ harnessId: string; modelId: string }>;
```

- [x] **Step 4: 在 orchestrator handleTurn 中插入 session lock 检查 + 分类逻辑**

插入点：`visibleHistory` 定义之后（L1854 之后）、`input.model` 被消费之前（L2000 之前）。实际窗口约 L1860–1999。

Turn 流程：

```
turn 开始
  ├─ 1. sessionLockStore.get(sessionId)
  │    命中 → 覆写 input.harness + input.model（screener 照常，分类器跳过）
  ├─ 2. security screener（现有）
  ├─ 3. 分类器（已锁会话跳过）
  │    L1 → sessionLockStore.set(sessionId, 具体值) + 覆写 input
  └─ 4. runTurn
```

```typescript
const sessionLock = deps.sessionLockStore
  ? await deps.sessionLockStore.get(input.session.id)
  : null;
...
if (sessionLock) {
  ...
  if (sensitivityVerdict?.route) {
    ...
    if (sensitivityVerdict.route.sessionPin && deps.sessionLockStore) {
      await deps.sessionLockStore.put(input.session.id, {
        harnessId: sensitivityVerdict.route.harnessId,
...
- [x] **Step 5: Fork 传播 — 拷贝 session lock**

`src/api/app-sessions.ts` 的 `forkSession` 方法中，在 entries 复制完毕、`releaseLease` 之后（约 L450），追加：
...
fork 继承源会话的全部历史，必须继承锁，否则敏感历史随新会话流出。

- [x] **Step 6: 写集成测试 — 全链路：分类 → session lock 写入 + 覆写**
...
test("classifier L1 writes session lock and overrides harness/model", async () => {
  ...
  // 断言 sessionLockStore 中存在该 session 的锁记录
  // 再发一条消息（手动指定不同模型），断言 lock 覆盖了手动选择
...
- L1 sidecar 响应 → `input.harness` + `input.model` 被覆写 + `sessionLockStore` 写入
- 已锁会话的第二条消息 → 分类器跳过，路由仍锁定
- 已锁会话手动选模型 → lock 覆盖手动选择
- sidecar 不可达 → fallback 对生效 + `sessionLockStore` 写入
- `CLASSIFIER_URL` 未设置 → 分类跳过
- Fork 会话 → lock 从源会话拷贝到新 session ID

- [x] **Step 7: 提交**

```bash
git add src/security/sensitivity-classifier.ts src/core/orchestrator.ts src/core/orchestrator/types.ts src/wiring.ts src/config.ts src/api/app-sessions.ts test/sensitivity-classifier.test.ts test/sensitivity-classifier-integration.test.ts
git commit -m "feat: integrate sensitivity classifier with session-level one-way lock"
```
...
- [x] **Step 1: 全链路集成测试（含 session lock 写入/读取/fork 拷贝 断言）**
...
- 已锁会话手动选模型 → lock 覆盖
- Fork → 锁拷贝到新 session
- sidecar 不可达 → fallback 锁
- `CLASSIFIER_URL` 未设置 → 跳过
- L3 + general → 不覆写、不写锁

- [x] **Step 2: 全项目 typecheck + lint**
