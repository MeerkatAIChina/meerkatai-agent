...
- 利用现有 `requested.modelId` 覆盖位和独立 session lock store（不被 UI 手动选模型覆盖）
...
会话级单向锁，独立于 `ScopedConfigStore` 的 `PersistedBaseModel`（后者会被 UI 手动选模型覆盖）。锁的存储是一个 key = session ID 的 `DurableMap<{ harnessId: string; modelId: string }>`，注入 orchestrator deps 和 app deps（用于 fork 拷贝）。

锁的内容是分类 verdict 中已解析的具体 `{harnessId, modelId}` 快照，不存逻辑名。路由配置后续变动不影响已锁会话。
...
turn 开始
  │
  ├─ 1. [NEW] sessionLockStore.get(sessionId)
  │    命中 → 覆写 input.harness + input.model
  │    （screener 照常执行，但分类器跳过——锁是隐私防线，screener 是注入防线，两条独立）
  │
  ├─ 2. [现有] security screener（posture 门控）
  │    输入: screenPayload(externalData)
  │    输出: SecurityScreenVerdict
  │
  ├─ 3. [NEW] 敏感度分类器（已锁会话跳过；CLASSIFIER_URL 未配置也跳过）
  │    输入: { text: userMessage + 最近 3 条消息原文, scopeId, orgScopeId, surface }
  │    输出: SensitivityVerdict | null
  │
  │    L1 → sessionLockStore.set(sessionId, { harnessId: verdict.route.harnessId, modelId: verdict.route.model })
  │          + 覆写 input.harness + input.model
  │
  └─ 4. runTurn
```
...
`src/api/app-sessions.ts` 约 +3 行：fork 创建新会话后，将原会话的锁拷贝到新 session id。fork 继承了被污染的历史，必须继承锁。

### 约束与注记

1. **锁单向不可逆**。会话一旦被判定 L1，该会话永久锁定本地模型，手动选模型无法覆盖（orchestrator 在 step 1 检查并覆写 input）。使用通用模型须另开新会话。
2. **锁存具体值**。锁的内容是 `{harnessId, modelId}` 快照，不存逻辑名。路由配置（`routes.jsonc`）后续变动不影响已锁会话。
3. **已锁会话跳过分类器**。分类结果不可能改变路由决策，省一次 HTTP round-trip。
4. **无 route = 不干预**。L3 + general 时 sidecar 不返回 `route` 字段，core 不做任何模型覆盖，走 org 默认的 `resolveRuntimeChoice`。

### 不改动的文件

- `src/security/security-posture.ts` — 不动，`SecurityScreenVerdict` 不加字段
- `src/security/security-screener.ts` — 不动，分类器独立解析
- `src/harness/harness-router.ts` — 不动，锁在 orchestrator 层覆写 `input.harness`/`input.model`，`requested.modelId` 覆盖和白名单校验已原生支持
...
sidecar 不可达/超时/5xx 时，core 直接用 `CLASSIFIER_FALLBACK_MODEL` + `CLASSIFIER_FALLBACK_HARNESS` 做 route 覆盖 + 写入锁，语义和 sidecar 返回 L1 完全一致。core 不持有逻辑名→具体 id 的映射——fallback 直接配具体值。
...
| 字段 | 含义 | Core 行为 |
| ------ | ------ | ---------- |
| `level` | 隐私敏感级 L1/L2/L3 | 决定是否加锁、是否需确认 |
...
| `route.harness_id` | 对应 harness | 加锁时写入 `sessionLockStore` |
| `route.session_pin` | 是否加会话锁 | 写入 session 级 lock store（durable），永不解锁 |
...
| 隐私级 | 领域 | 触发条件 | policy | model | 会话锁 | 确认 |
| ------ | ---- | -------- | ------ | ----- | ------ | ---- |
| L1 | - | PII / scope 白名单 / 语义高置信 | `local-secure` | meerkat-triz-v1 | ✅ 单向锁 | 无 |
| L2 | - | 语义中置信 | `local-secure` | meerkat-triz-v1 | ✅ 单向锁 | 需确认（首版降 L1） |
| L3 | TRIZ | TRIZ 关键词 / 语义命中 | `meerkat-triz-v1` | meerkat-triz-v1 | ❌ | 无 |
| L3 | general | 默认 | 无 route | 不干预 | - | - |

关键区别：TRIZ 是**能力路由**不是隐私路由。TRIZ 命中但无敏感数据 → 直接走 TRIZ 模型，不锁、不确认——下一条消息可能是电商文案，自动回通用模型。L3 + general 不返回 route，core 不干预模型选择。

L1/L2 的会话锁是单向不可逆的——一旦触发，该会话永久锁定本地模型，手动选模型无法覆盖，fork 继承锁，唯一解除方式是开新会话。

### 降级策略

| 场景 | 行为 | 审计 |
| ------ | ------ | ------ |
| Sidecar 连接拒绝 | → 按 `CLASSIFIER_FALLBACK_*` 路由 + 写入锁 | `classifier.unavailable` |
| Sidecar 超时（2s） | → 按 `CLASSIFIER_FALLBACK_*` 路由 + 写入锁 | `classifier.timeout` |
| Sidecar 返回 5xx | → 按 `CLASSIFIER_FALLBACK_*` 路由 + 写入锁 | `classifier.error` |
...
- 锁写 `sessionLockStore.set(sessionId, {harnessId, modelId})`（Postgres durable），不用内存版；独立于 `ScopedConfigStore` 的 `PersistedBaseModel`，避免被 UI 手动选模型覆盖
...
- L1 会话锁定到 `local-secure` 后，Agent 仍然看到完整的 skill index
...
| fail-to-local 三分支 | Sidecar 拒绝/超时/5xx → 断言 `CLASSIFIER_FALLBACK_*` 路由生效 + 写入锁 |
| CLASSIFIER_URL 未设置 | 断言跳过分类，turn 正常执行 |
| Session lock 写入/覆盖 | L1 turn → 断言 `sessionLockStore` 有该 session 记录；已锁会话手动选模型 → 断言 lock 覆盖 |
| Fork 传播 | Fork 有锁的会话 → 断言新 session 的 lock 与源相同 |
