...
export function defaultSessionTitle(s: CoreSession): string {
  ...
  if (surface === "web") return String(i18n("Web chat"));
  if (s.type === "channel") return channelLabel(s) ?? String(i18n("Channel"));
  if (s.type === "group") return groupDmText(s.channelName) ?? s.channelName?.trim() ?? String(i18n("Group DM"));
  return String(i18n("Direct message"));
}
  ...
  render(
    html`
      ${
        pinned.length
          ? html`
              <div class="recents-group pinned-head">${icon(Pin, 11)}<span>${i18n("Pinned")}</span></div>
              ${repeat(
                pinned,
      ...
      ${sessionsLoading && visible.length === 0 ? html`<div class="empty" style="padding:16px">${i18n("Loading conversations...")}</div>` : ""}
      ${
        !sessionsLoading && !sessionsNotice && visible.length === 0
          ? html`<div class="empty" style="padding:16px">
              ${sessionsState.list.length ? i18n("Slack conversations hidden.") : i18n("No conversations yet.")}
            </div>`
          : ""
      }
    `,
...
function recentItem(item: RecentItem): TemplateResult {
  ...
  let fallbackName = String(i18n("Project"));
  if (item.groupKind === "channel") fallbackName = String(i18n("Channel"));
  else if (item.groupKind === "group") fallbackName = String(i18n("Group DM"));
  ...
  return html`
    <section class="recent-project ${item.sessions.some(isActiveRow) ? "active" : ""}" aria-label=${tr("{name} project")(name)}>
      ${
          ...
          : html`<div class="recent-project-head">
              ...
              <div class="session-menu recent-project-menu ${menuOpen ? "menu-open" : ""}">
                ...
                <button
                  ...
                  type="button"
                  title=${i18n("Project options")}
                  aria-label=${tr("Options for {name}")(name)}
                  aria-haspopup="menu"
              ...
              <button
                ...
                type="button"
                aria-label=${tr("New chat in {name}")(name)}
                title=${tr("New chat in {name}")(name)}
...
function projectMenuPopover(item: Extract<RecentItem, { kind: "project" }>): TemplateResult {
  ...
  return html`
    <div class="session-menu-popover" role="menu" ${ref(placeSessionMenu)} @click=${(e: Event) => e.stopPropagation()}>
      ...
      >
        ${icon(Folder, 15)}<span>${i18n("View project")}</span>
      </button>
      ${
        owned
          ? html`<button
            ...
            >
              ${icon(Pencil, 15)}<span>${i18n("Rename")}</span>
            </button>`
          : nothing
      }
    </div>
...
function projectRenameRow(item: Extract<RecentItem, { kind: "project" }>): TemplateResult {
  ...
  return html`<div class="recent-project-head renaming">
    ${renameInput(menuKey, String(i18n("Rename project")), () => commitProjectRename(item))}
  </div>`;
}
...
export function drawChatsPage(): void {
  ...
  const rows = [...sessionsState.list]
    ...
    .map((s) => chatPageRow(s));
  let empty = String(i18n("No conversations yet — start a new chat."));
  if (sessionsLoading && sessionsState.list.length === 0) empty = String(i18n("Loading conversations…"));
  else if (chatsPageScope || q || chatsPageStatus !== "active" || chatsPageSurface !== "all") {
    empty = String(i18n("No conversations match."));
  }
  render(
    listPageTpl({
      title: String(i18n("Chats")),
      ...
      action: { label: String(i18n("New chat")), onClick: () => mainConversation().newChat() },
      search: {
        value: chatsPageQuery,
        placeholder: String(i18n("Search chats…")),
        onInput: (v) => {
          chatsPageQuery = v;
          drawChatsPage();
        },
      ...
      filters: html`<div class="chat-filters">
        <div class="resource-tabs" role="tablist" aria-label=${i18n("Conversation status")}>
          ${(
            [
              ["active", String(i18n("Active"))],
              ["waiting", String(i18n("Waiting"))],
              ["archived", String(i18n("Archived"))],
            ] as const
          ).map(
            ([value, label]) =>
              html`<button
        ...
        <label class="list-select"
          ><span>${i18n("Surface")}</span>${fieldSelect({
            compact: true,
            ...
            options: [
              html`<option value="all">${i18n("All surfaces")}</option>`,
              html`<option value="web">${i18n("Web")}</option>`,
              html`<option value="slack">${i18n("Slack")}</option>`,
            ],
          })}</label
        >
      </div>`,
...
function chatMatches(s: CoreSession, q: string): boolean {
  const context = sharedContextLabel(s.scopeId, s.channelName ?? null) ?? String(i18n("Personal"));
...
function statusMarks(s: CoreSession): TemplateResult {
  ...
  return html`${ind.working ? html`<span class="working-dot" ${ref(syncWorkingPulse)} title=${i18n("Agent is working")} aria-label=${i18n("Agent is working")}></span>` : nothing}${
    ind.awaiting
      ? html`<span class="awaiting-dot" title=${i18n("Waiting for your reply")} aria-label=${i18n("Waiting for your reply")}></span>`
      : nothing
  }${
    ind.background
      ? html`<span
          ...
          role="button"
          tabindex="0"
          aria-label="${ind.background.label} — ${String(i18n("click to inspect"))}"
          @mouseenter=${(e: Event) =>
            showTooltip(e.currentTarget as Element, `${ind.background!.label} — ${String(i18n("click to inspect"))}`)}
          @mouseleave=${(e: Event) => hideTooltip(e.currentTarget as Element)}
          @focus=${(e: Event) => showTooltip(e.currentTarget as Element, `${ind.background!.label} — ${String(i18n("click to inspect"))}`)}
    ...
    >
      ...
      >
        ...
        <span class="list-row-meta">
          ...
          ${readOnly ? html`<span class="ro-lock" title=${i18n("Read-only")}>${icon(Lock, 12)}</span>` : nothing}
        ...
        s.id
          ? html`<span class="chat-row-actions">
              <button
                ...
                aria-label=${tr("Copy link to {title}")(sessionTitle(s))}
              ...
              <button
                class="icon-btn"
                type="button"
                title=${s.pinned ? i18n("Unpin") : i18n("Pin")}
                aria-label=${`${s.pinned ? String(i18n("Unpin")) : String(i18n("Pin"))} ${sessionTitle(s)}`}
                @click=${() => {
              ...
              <button
                class="icon-btn"
                type="button"
                title=${s.archived ? i18n("Unarchive") : i18n("Archive")}
                aria-label=${`${s.archived ? String(i18n("Unarchive")) : String(i18n("Archive"))} ${sessionTitle(s)}`}
                @click=${() => {
...
function sessionRow(s: CoreSession, projectChild = false): TemplateResult {
  ...
  if (untitledProjectChild) title = surfaceOf(s) === "web" ? String(i18n("Web chat")) : String(i18n("New chat"));
  const readOnly = !isContinuable(s, appState.me?.user ?? "");
  const surface = surfaceOf(s);
  ...
  const ariaLabel = [
    ...
    context,
    working ? String(i18n("agent is working")) : null,
    s.awaitingInput ? String(i18n("waiting for your reply")) : null,
    readOnly ? String(i18n("read-only")) : null,
    s.pinned ? String(i18n("pinned")) : null,
    ...
    >
      ...
      >
        <div class="title" aria-live="polite">
          ${statusMarks(s)}${surfaceGlyph(s)}${readOnly ? html`<span class="ro-lock" title=${i18n("Read-only")}>${icon(Lock, 12)}</span>` : nothing}<span
            class="tl"
            >${titleContent}</span
        ...
        saved
          ? html`<div class="session-menu">
              ...
              <button
                ...
                data-menu-id=${s.id}
                type="button"
                title=${i18n("Conversation options")}
                aria-haspopup="menu"
                aria-expanded=${menuOpen ? "true" : "false"}
  ...
  return html`
    <div class="session-menu-popover" role="menu" ${ref(placeSessionMenu)} @click=${(e: Event) => e.stopPropagation()}>
      <button class="session-menu-option" type="button" role="menuitem" @click=${() => void copySessionLink(s)}>
        ${icon(Link, 15)}<span>${i18n("Copy link")}</span>
      </button>
      <button class="session-menu-option" type="button" role="menuitem" @click=${() => setPinned(s, !pinned)}>
        ${pinned ? icon(PinOff, 15) : icon(Pin, 15)}<span>${pinned ? i18n("Unpin") : i18n("Pin")}</span>
      </button>
      ...
      >
        ${icon(RefreshCw, 15)}<span>${refreshingTitle ? i18n("Refreshing title") : i18n("Refresh title")}</span>
      </button>
      <button class="session-menu-option" type="button" role="menuitem" @click=${() => setArchived(s, !archived)}>
        ${archived ? icon(ArchiveRestore, 15) : icon(Archive, 15)}<span>${archived ? i18n("Unarchive") : i18n("Archive")}</span>
      </button>
      ${sessionColorRow(s)}
    </div>
  `;
...
function sessionColorRow(s: CoreSession): TemplateResult {
  ...
  return html`
    <div class="session-menu-colors" role="group" aria-label=${i18n("Row color")}>
      ${SESSION_COLORS.map(
        (c) => html`
          <button
            ...
            type="button"
            style=${`--swatch:${c}`}
            title=${tr("Color row {color}")(c)}
            aria-label=${tr("Color row {color}")(c)}
            aria-pressed=${current === c ? "true" : "false"}
            @click=${() => setColor(s, current === c ? null : c)}
          ></button>
        `,
      )}
      <label class="color-swatch custom ${current && !isPreset ? "selected" : ""}" title=${i18n("Custom color (RGB picker)")}>
        <input
          type="color"
          aria-label=${i18n("Custom row color")}
      ...
      ${
        current
          ? html`<button
              ...
              type="button"
              title=${i18n("Clear color")}
              aria-label=${i18n("Clear row color")}
...
function renameRow(s: CoreSession): TemplateResult {
  return html`<div class="session-row renaming">
    ${renameInput(s.id, String(i18n("Rename conversation")), () => commitRename(s))}
  </div>`;
}
...
async function runSessionsRefresh(
  ...
  try {
    ...
    if (!opts.silent) sessionsNotice = errMessage(e, String(i18n("Failed to load conversations.")));
    return false;
  } finally {
