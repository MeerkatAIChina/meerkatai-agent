...
export function renderSignInEmail(args: {
  ...
  const text = [
    ...
    link,
    "",
    `此链接只能使用一次，${args.ttlMinutes} 分钟后过期。请在你发起登录的浏览器中打开。`,
    "如果这不是你发起的登录，忽略本邮件即可 — 在你确认之前不会发生任何操作。",
  ].join("\n");
  const html = `<!doctype html>
<html lang="zh-CN"><head><meta charset="utf-8"><meta name="color-scheme" content="light"></head>
...
<tr><td>
<h1 style="margin:0 0 10px;font-size:20px;font-weight:600">登录 ${escapeHtml(brand)}</h1>
<p style="margin:0 0 24px;color:#525252">点击下方按钮完成登录。链接只能使用一次，${args.ttlMinutes} 分钟后过期，并请在你发起登录的浏览器中打开。</p>
<p style="margin:0 0 24px"><a href="${escapeHtml(link)}" style="display:inline-block;background:#0a0a0a;color:#ffffff;text-decoration:none;font-weight:600;padding:13px 22px;border-radius:10px">登录</a></p>
<p style="margin:0 0 8px;color:#737373;font-size:13px">或将此地址粘贴到浏览器中：</p>
<p style="margin:0 0 24px;word-break:break-all;font-size:12px;color:#525252">${escapeHtml(link)}</p>
<p style="margin:0;color:#737373;font-size:13px">如果这不是你发起的登录，忽略本邮件即可 — 在你确认之前不会发生任何操作。</p>
</td></tr></table>
</td></tr></table>
</body></html>`;
  return { to: args.to, subject: `登录 ${brand}`, text, html };
}
