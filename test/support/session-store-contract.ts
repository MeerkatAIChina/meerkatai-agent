...
import type { SessionStore, StoreOptions } from "../../src/sessions/session-store.ts";
import { scopeId } from "../../src/types.ts";

export function runSessionStoreContract(name: string, makeStore: (opts?: StoreOptions) => SessionStore): void {
  test(`${name}: attributedTurns applies each window's [validFrom,validTo) and excludes overheard`, async () => {
  ...
  test(`${name}: a blocked acquire names, dates, and times out the holder it lost to`, async () => {
    const nowRef = { v: 10_000_000_000 };
    const t0 = nowRef.v;
